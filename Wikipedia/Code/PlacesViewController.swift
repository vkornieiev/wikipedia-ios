...
class PlacesViewController: ViewController, UISearchBarDelegate, ArticlePopoverViewControllerDelegate, PlaceSearchSuggestionControllerDelegate, NSFetchedResultsControllerDelegate, UIPopoverPresentationControllerDelegate, ArticlePlaceViewDelegate, UIGestureRecognizerDelegate, HintPresenting {
    ...
    @objc public func showArticleByLocation(_ location: CLLocation, title: String?) {
        let displayTitle = title ?? location.isoFormattedString
        let region = region(thatFits: [location.coordinate])
        let searchResult = MWKSearchResult(articleID: 0, revID: 0, title: title ?? "", displayTitle: displayTitle, displayTitleHTML: nil, wikidataDescription: nil, extract: nil, thumbnailURL: nil, index: nil, titleNamespace: nil, location: location)
        currentSearch = PlaceSearch(filter: .top, type: .location, origin: .user, sortStyle: .links, string: nil, region: region, localizedDescription: displayTitle, searchResult: searchResult, siteURL: nil)
    }
